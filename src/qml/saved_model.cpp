...
int SavedTimetableModel::find( int timetableId )
{
    auto iter = std::find_if( m_timetables.begin(), m_timetables.end(),
       [=]( const auto& it ) { return it.id == timetableId; } );
...
{
    auto it = std::find_if( m_timetables.begin(), m_timetables.end(),
       [=]( const internal::SavedTimetable& s ) { return s.id == id; } );
    if( it != m_timetables.end() )
    {
        beginRemoveRows( {}, std::distance( m_timetables.begin(), it ),
           std::distance( m_timetables.begin(), it ) );
...
QVariant SavedTimetableModel::data( const QModelIndex& index, int role ) const
{
    ...
    case TimetableRoles::LAST_UPDATE:
    {
        ...
        {
            return QDateTime::fromString( item.lastUpdate )
...
{
    QHash< int, QByteArray > roles{ { TimetableRoles::ID, "id" },
       { TimetableRoles::TITLE, "title" }, { TimetableRoles::LAST_UPDATE, "lastUpdate" },
