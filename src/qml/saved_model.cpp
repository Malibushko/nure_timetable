...
{
   auto iter =
      std::find_if( m_timetables.begin(), m_timetables.end(), [=]( const auto& it ) { return it.id == timetableId; } );
...
void SavedTimetableModel::replaceItem( const QVariant& item )
{
   ...
   auto             iter = std::find_if( m_timetables.begin(), m_timetables.end(),
      [&]( const internal::SavedTimetable& t ) { return t.id == timetable.id; } );
...
{
   auto it = std::find_if(
      m_timetables.begin(), m_timetables.end(), [=]( const internal::SavedTimetable& s ) { return s.id == id; } );
...
QVariantList SavedTimetableModel::getHideLessons( int id )
{
   for( auto& it : m_timetables )
   {
      if( it.id == id )
      {
         QVariantList list;
         list.reserve( it.lessonHide.size() );
         for( const auto& item : it.lessonHide )
...
QVariant SavedTimetableModel::createTimetable( int id, const QVariantList& hideItems )
{
   for( auto& it : m_timetables )
   {
      if( it.id == id )
      {
         it.lessonHide.clear();
         for( const auto& item : hideItems )
            it.lessonHide.push_back( qvariant_cast<internal::LessonHide>( item ) );
...
QVariant SavedTimetableModel::data( const QModelIndex& index, int role ) const
{
   ...
   switch( role )
   {
      ...
      case TimetableRoles::LAST_UPDATE:
      {
         ...
         if( auto updateDate = QDateTime::fromString( item.lastUpdate ); updateDate.daysTo( today ) < 2 )
         {
            return dayToString( updateDate );
         }
         else
         {
            return QDateTime::fromString( item.lastUpdate ).toString( "yyyy-MM-dd hh:mm" );
         }
      }
      ...
      case TimetableRoles::HIDE_LESSONS:
      {
         QVariantList list;
         list.reserve( item.lessonHide.size() );
         for( const auto& lesson : item.lessons )
            list.push_back( QVariant::fromValue( lesson ) );
         return QVariant::fromValue( list );
      }
...
{
   QHash<int, QByteArray> roles{ { TimetableRoles::ID, "id" }, { TimetableRoles::TITLE, "title" },
      { TimetableRoles::LAST_UPDATE, "lastUpdate" }, { TimetableRoles::LESSONS, "lessons" },
      { TimetableRoles::IS_TEACHER, "isTeacher" }, { TimetableRoles::TIME_SINCE_LAST_UPDATE, "timeSinceLastUpdate" },
      { TimetableRoles::HIDE_LESSONS, "hideLessons" } };
   return roles;
}
