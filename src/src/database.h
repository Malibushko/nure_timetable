...
private:
   ...
   {
      QEventLoop waiter;
      m_executor.submit( []() { return QVariant::fromValue( Database::connect() ); } ).then( [&]( QVariant table ) {
...
public:
   ...
   void Get( TableType type, Callback&& cb = {} )
   {
      using namespace internal;
      QueryBuilder builder;
      builder.select().table( m_tableName ).where( QString( "type = %1" ).arg( static_cast<int>( type ) ) );
      m_executor
         .submit( [query = builder.query()]() {
            QSqlQuery    q( query );
            QVariantList result;
            while( q.next() )
            {
               result.push_back( QVariant::fromValue(
                  JsonParser::i().fromJson<Resource>( q.value( "data" ).toString().toStdString().data() ) ) );
            }
   ...
   {
      m_executor.submit( []() { return QVariant::fromValue( QSqlDatabase::database().transaction() ); } );
   }
   /**
   ...
   void commit()
   {
      m_executor.submit( [] { return QVariant::fromValue( QSqlDatabase::database().commit() ); } )
   ...
   void rollback()
   {
      m_executor.submit( [] { return QVariant::fromValue( QSqlDatabase::database().rollback() ); } )
         .then( DefaultCallback{} );
   }
   /**
