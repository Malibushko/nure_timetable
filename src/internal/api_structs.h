...
struct Department
{
   declare_struct( Department ) declare_field( int_t, id ); /**< id of department */
...
struct Teacher
{
   declare_struct( Teacher ) declare_field( int_t, id ); /**< teacher id */
...
struct Faculty
{
   declare_struct( Faculty ) declare_field( int_t, id );  /**< faculty id */
...
struct AuditoryType
{
   declare_struct( AuditoryType ) declare_field( string_t, id ); /**<  Auditory id */
...
struct Auditory
{
   declare_struct( Auditory ) declare_field( string_t, id );   /**<  Auditory id */
   declare_field( string_t, short_name );                      /**<  Auditory short name */
...
struct University
{
   declare_struct( University ) declare_field( string_t, short_name ); /**< Short name of the university */
...
struct Group
{
   declare_struct( Group ) declare_field( int_t, id ); /**<  Group id*/
...
struct Speciality
{
   declare_struct( Speciality ) declare_field( int_t, id ); /**< Speciality id */
...
struct Direction
{
   declare_struct( Direction ) declare_field( int_t, id ); /**< Direction id */
   ...
   declare_field(
      container_t<Speciality>, specialities ); /**<  Direction specialities \note Can contain another groups*/
};
/**
...
{
   declare_struct( UniversityWrapper ) declare_field( University, university ); /**<  University object */
};
/**
...
{
   declare_struct( DepartmentWrapper ) declare_field( Department, department ); /**< Department object */
};
/**
...
{
   declare_struct( FacultyWrapper ) declare_field( Faculty, faculty ); /**<  */
};
/**
...
struct Timetable
{
   declare_struct( Timetable ) declare_field( int, id ); /**<  Timetable id */
   declare_field( string_t, title );                     /**<  Timetable title */
};

struct LessonHide
{
   declare_struct( LessonHide );
   declare_field( string_t, subject );
   declare_field( uint16_t, data );
};

struct LessonNote
{
   declare_struct( LessonNote );
   declare_field( int, index );
   declare_field( QString, note );
   declare_field( QString, lastWriteDate );
};
struct Notes
{
   declare_struct( Notes );
   declare_field( int, id );
   declare_field( std::vector<LessonNote>, notes );
};
...
struct Lesson
{
   ...
   static Lesson fromCSV( const QString& csvLine )
   {
      ...
      if( innerList.size() < 4 )
         return {};
      innerList.erase(
         std::remove_if( innerList.begin(), innerList.end(), []( QString string ) { return string.size() <= 1; } ),
         innerList.end() );
...
struct SavedTimetable
{
   ...
   declare_field( std::vector<LessonHide>, lessonHide );
};

struct TableHideSettings
{
   string_t                          lessonName;
   std::set<string_t>                lessonTypes;
   std::set<std::pair<QTime, QTime>> lessonTime;
};
} // namespace timetable::internal
...
Q_DECLARE_METATYPE( timetable::internal::Notes )
Q_DECLARE_METATYPE( timetable::internal::Lesson )
Q_DECLARE_METATYPE( timetable::internal::SavedTimetable )
Q_DECLARE_METATYPE( timetable::internal::LessonNote )
Q_DECLARE_METATYPE( timetable::internal::TableHideSettings )
Q_DECLARE_METATYPE( timetable::internal::LessonHide )
